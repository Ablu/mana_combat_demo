            ...
            if let Some(tiled_map) = maps.get(map_handle) {
                ...
                for (tileset_index, tileset) in tiled_map.map.tilesets().iter().enumerate() {
                    ...
                    for (layer_index, layer) in tiled_map.map.layers().enumerate() {
                        ...
                        let typ = match layer.layer_type() {
                            tiled::LayerType::Tiles(_) => "tiles",
                            tiled::LayerType::Objects(_) => "objects",
                        ...
                        let tiled::LayerType::Tiles(tile_layer) = layer.layer_type() else {
                            log::info!(
                                "Skipping layer {} because only tile layers are supported (was: {:?}).",
                                layer.id(),
                                typ
                            );
                            continue;
                        ...
                        // skip automapping layers
                        if layer.name.starts_with("set") {
                            continue;
                        }
